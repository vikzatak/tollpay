...
namespace DPTPWebAPI.Controllers
{
    ...
    public class ecom_RFIDController : ApiController
    {
        ...
        public HttpResponseMessage GetFASTagListbyDistibutor(HttpRequestMessage request, string distributorid)
        {
            ...
            var ecom = db.ecom_RFID.Where(r => r.ecom_DistributionID == distributorid).Select(x => new { CustMobNo = x.ecom_CustomerMobNo, CustVRN = x.ecom_CustomerVehicleNo, CustFASTag = x.ecom_RFIDTagSrNo, CustTagSrNo = x.Serial_Number, CustTagStatus = x.ecom_RFIDStatus, CustWallet = x.ecom_axis_WalletID }).ToList();
            return request.CreateResponse(HttpStatusCode.OK, ecom);
        }
        // GET: api/ecom_RFID1
        //[JwtAuthentication]
        ...
        public HttpResponseMessage DistributorToSalesPersonFastTags(HttpRequestMessage request, DistributorToSalesPersonTags[] dtspt)
        {
            ...
            try
            {
                foreach (var item in dtspt)
                {
                    ...
                    {
                        db.ecom_RFID.Where(ec => ec.Serial_Number == item.TagNo && ec.ecom_DistributionID == item.DistributorUserID).FirstOrDefault().ecom_DistributionID = item.SalesUserID;
                        db.ecom_RFID.Where(ec => ec.Serial_Number == item.TagNo && ec.ecom_DistributionID == item.DistributorUserID).FirstOrDefault().ecom_CreatedDate = DateTime.Now;
                        db.SaveChanges();
                    }
                }
